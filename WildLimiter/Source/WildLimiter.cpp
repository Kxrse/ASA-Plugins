...
static bool LoadConfig()
{
    ...
    try
    {
        ...
        Log::GetLog()->info("[WildLimiter] Config loaded, {} blocked, {} caps, {} overrides",
            g_def_blocked.size(), g_def_caps.size(), g_overrides.size());
        return true;
    }
    catch (const std::exception& ex)
    {
        Log::GetLog()->error("[WildLimiter] Config parse error: {}", ex.what());
        return false;
    }
}
...
static void SweepBlocked()
{
    ...
    if (!targets.empty())
        Log::GetLog()->info("[WildLimiter] Sweep removed {} blocked wild dinos", targets.size());
}
...
static void PluginInit()
{
    ...
    if (!LoadConfig())
        Log::GetLog()->error("[WildLimiter] Failed to load config");
    ...
    Log::GetLog()->info("[WildLimiter] Loaded");
}
...
static void PluginUnload()
{
    ...
    Log::GetLog()->info("[WildLimiter] Unloaded");
}
