 ...
 *   "DbPass": "Password",
 *   "DbName": "arkcluster",
 *   "CryopodBlueprint": "Blueprint'/Game/PrimalEarth/CoreBlueprints/Weapons/PrimalItem_WeaponEmptyCryopod.PrimalItem_WeaponEmptyCryopod'",
 *   "PelayoriCryopodBlueprint": "Blueprint'/Cryopods/Cryopods/PrimalItem_WeaponEmptyCryopod_Mod.PrimalItem_WeaponEmptyCryopod_Mod'",
 *   "UsePelayoriCryo": false,
 ...
 *       "Items": [
 *         {
 *           "BlueprintPath": "Blueprint'/Game/PrimalEarth/CoreBlueprints/Items/Consumables/PrimalItemConsumable_Berry_Mejoberry.PrimalItemConsumable_Berry_Mejoberry'",
 *           "Quantity": 100,
 *           "Quality": 0.0
 *         }
 *       ],
 ...
 * groups with per group cooldowns and use caps persisted in MariaDB. Permissions is
 * resolved lazily on the first group lookup because plugin load order is alphabetical and
 * Permissions.dll is not present at Kits init. Each kit may carry a unique Number so
 * players can redeem with /kit 1 as well as /kit name. Item quantities larger than the
 * server stack size are split across multiple stacks. Dinos are spawned, serialised into
 * a cryopod, then destroyed so the player receives a cryopodded dino. Before each redeem
 * the DB link is pinged and reopened if it dropped, and the connection is dropped for a
 * fresh reconnect if the config credentials change.
 */
...
static bool LoadMySQLLib()
{
    ...
    if (!pmysql_init || !pmysql_real_connect || !pmysql_close ||
        !pmysql_query || !pmysql_error || !pmysql_real_escape_string || !pmysql_fetch_row || !pmysql_ping)
    {
...
static int g_config_reload_ticks = 0;

static MYSQL* g_db = nullptr;
static std::mutex g_db_mutex;

static std::string g_conn_host;
static std::string g_conn_user;
static std::string g_conn_pass;
static std::string g_conn_name;
static unsigned int g_conn_port = 0;
...
static void CheckConfigReload()
{
    ...
    if (g_db && (g_db_host != g_conn_host || g_db_user != g_conn_user ||
                 g_db_pass != g_conn_pass || g_db_name != g_conn_name ||
                 g_db_port != g_conn_port))
    {
        std::lock_guard<std::mutex> lock(g_db_mutex);
        pmysql_close(g_db);
        g_db = nullptr;
        Log::GetLog()->info("[Kits] DB credentials changed, will reconnect on next use");
    }
}
...
static bool OpenConnection()
{
    ...
    g_conn_host = g_db_host;
    g_conn_user = g_db_user;
    g_conn_pass = g_db_pass;
    g_conn_name = g_db_name;
    g_conn_port = g_db_port;
...
static bool EnsureConnected()
{
    ...
    Log::GetLog()->info("[Kits] DB reconnected");
    return true;
}
...
static bool g_permissions_warned = false;

static bool EnsurePermissionsAPI()
{
    if (g_permissions_loaded) return true;

    HMODULE hMod = GetModuleHandleA("Permissions");
    if (!hMod)
    {
        if (!g_permissions_warned)
        {
            g_permissions_warned = true;
            Log::GetLog()->warn("[Kits] Permissions.dll not loaded, falling back to default group");
        }
        return false;
    }

    GetPlayerGroups_t fn = (GetPlayerGroups_t)GetProcAddress(hMod,
    ...
    if (!fn)
    {
        ...
        {
            g_permissions_warned = true;
            Log::GetLog()->warn("[Kits] Failed to resolve Permissions GetPlayerGroups, falling back to default group");
        }
        return false;
    }
...
static TArray<float> GetCharacterStatsAsFloats(APrimalDinoCharacter* dino)
{
    ...
    floats.Add(static_cast<float>(len));
    return floats;
}
...
static void TimerConfigReload()
{
    if (++g_config_reload_ticks < 10) return;
    g_config_reload_ticks = 0;
    CheckConfigReload();
}
...
static void PluginInit()
{
    ...
    AsaApi::GetCommands().AddOnTimerCallback(FString(L"Kits.ConfigReload"), &TimerConfigReload);
...
static void PluginUnload()
{
    ...
    AsaApi::GetCommands().RemoveOnTimerCallback(FString(L"Kits.ConfigReload"));
